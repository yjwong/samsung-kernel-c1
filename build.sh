...
build ()
{
    ...
    [ x = "x$NO_DEFCONFIG" ] && make -C "$KERNEL_DIR" O="$target_dir" c1_${target}_defconfig HOSTCC="$CCACHE gcc" -j8
    if [ x = "x$NO_BUILD" ] ; then
        make -C "$KERNEL_DIR" O="$target_dir" HOSTCC="$CCACHE gcc" CROSS_COMPILE="$CCACHE $CROSS_PREFIX" -j8 modules
        make -C "$KERNEL_DIR" O="$target_dir" HOSTCC="$CCACHE gcc" CROSS_COMPILE="$CCACHE $CROSS_PREFIX" -j8 zImage
